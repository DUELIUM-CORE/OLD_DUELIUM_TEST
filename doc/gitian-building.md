...
*Setup instructions for a gitian build of Duelium using a Debian VM or physical system.*
...
- [Building Duelium](#building-Duelium)
...
Clone the git repositories for Duelium and gitian and then checkout the Duelium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Duelium repository
...
To build Duelium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Duelium repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Duelium/.git/
...
For example:
```bash
URL=https://github.com/konez2k/Duelium.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Duelium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Duelium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Duelium-build.assert
```
...
[Duelium/gitian.sigs](https://github.com/Duelium/gitian.sigs/) repository, or if that's not possible to create a pull
request.
